...
*/
%type <ptr> program ExtDefList ExtDef Specifier ExtDecList FuncDec ArrayDec CompSt VarList VarDec ParamDec Stmt StmList DefList Def DecList Dec Exp Args StructBlock VarDecl 
...
%token STRUCT PLUS MINUS STAR DIV ASSIGNOP AND OR NOT IF ELSE WHILE RETURN COMADD COMSUB FOR //+=comadd，-=comsub，for，=assignop
//优先级
%left COMADD COMSUB
%left ASSIGNOP
%left OR
...
ExtDefList: {$$=NULL;}/*整个语法树为空*/
            ...
            ;
/*外部声明，声明外部变量或者声明函数*/
ExtDef:   STRUCT ID LC StructBlock RC SEMI {$$=mknode(EXT_STRUCT_DEF,$4,NULL,NULL,yylineno);strcpy($$->type_id,$2);}
        | Specifier ExtDecList SEMI {$$=mknode(EXT_VAR_DEF,$1,$2,NULL,yylineno);}//该结点对应一个外部变量声明
        ...
        ;
StructBlock: VarDecl {$$=mknode(STRUCTDECL,$1,NULL,NULL,yylineno);}
              | VarDecl StructBlock {$$=mknode(STRUCTDECL,$1,$2,NULL,yylineno);}
              ;
VarDecl: Specifier VarDec SEMI {$$=mknode(VARDECL,$1,$2,NULL,yylineno);}
        |  error SEMI {$$=NULL; printf("---缺少分号---\n");}
        ;

/*表示一个类型，int、float和char*/
Specifier: TYPE {$$=mknode(TYPE,NULL,NULL,NULL,yylineno);strcpy($$->type_id,$1);$$->type=(!strcmp($1,"int")?INT:(!strcmp($1,"float")?FLOAT:STRING));}
           ;
